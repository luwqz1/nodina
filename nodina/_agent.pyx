...
cdef set _node_dependencies(object node, object local_scope):
    ...
    for dependency in node.__dependencies__:
        dep = local_scope.retrieve(dependency).unwrap_or(_nodep)

        if dep is not _nodep:
...
cdef set _either_dependencies(object node, object local_scope, object winner):
    ...
    for candidate in node.__either__:
        dep = local_scope.retrieve(candidate).unwrap_or(_nodep)
...
def _compose_node(node, node_scope, local_scope, async_mode, winner=None):
    cached = node_scope.retrieve(node).unwrap_or(_nodep)

    if cached is not _nodep:
        return kungfu.Ok(cached)
...
def _compose_either(agent, node, node_scope, local_scope, mapped_scopes, results, async_mode):
    ...
    for dependency in node.__either__:
        result = results.get(dependency, _nodep)
...
def _compose_from_results(agent, node, node_scope, local_scope, mapped_scopes, results, async_mode):
    if issubclass(node, ResultNode):
        dep_result = results.get(node.__from_node__, _nodep)

        if dep_result is _nodep:
...
class _BaseNodinaAgent(Agent):
    ...
    def __repr__(self):
        return (
            f"<{type(self).__module__}.{type(self).__name__} traversed_nodes:"
            f" {self.traversed_nodes!r}; final_nodes={self.final_nodes!r}>"
        )
...
class NodinaAgent(_BaseNodinaAgent):
...
class AsyncNodinaAgent(_BaseNodinaAgent):
